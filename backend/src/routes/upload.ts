...
router.post('/profile', authMiddleware, localUploadMiddleware, async (req: Request, res: Response) => {
    try {
...
router.post('/profile/s3', authMiddleware, s3UploadMiddleware, async (req: Request, res: Response) => {
    try {
        const files = req.files as Express.Multer.File[]
        if (!files || files.length === 0) {
            return res.status(400).json({ status: false, message: '没有文件被上传' })
